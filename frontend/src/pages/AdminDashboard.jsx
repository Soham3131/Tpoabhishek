...
// import { PlusIcon, UsersIcon, DocumentTextIcon, ChartBarIcon } from '@heroicons/react/24/solid';
...
//   const [totalApplications, setTotalApplications] = useState(null); // State for total applications
...
//     return <div className="container mx-auto px-6 py-8 text-center text-lg">Loading Admin Dashboard...</div>;
//   }
...
//       {user && <p className="text-lg text-gray-700 mb-6">Welcome, {user.name} ({user.role})!</p>}
...
//             <div
//               className="bg-white p-6 rounded-lg shadow-md cursor-pointer hover:shadow-lg transition-shadow duration-300 flex flex-col items-center justify-center text-center"
//               onClick={handleViewAllUsers}
//             >
//               <UsersIcon className="w-12 h-12 text-primary mb-3" />
//               <h2 className="text-xl font-semibold text-secondary mb-2">Total Users</h2>
//               <p className="text-4xl font-bold text-primary">{dashboardData.stats.totalUsers}</p>
...
//               <DocumentTextIcon className="w-12 h-12 text-green-600 mb-3" />
//               <h2 className="text-xl font-semibold text-secondary mb-2">Verified Users</h2>
//               <p className="text-4xl font-bold text-green-600">{dashboardData.stats.verifiedUsers}</p>
//             </div>
//             <div className="bg-white p-6 rounded-lg shadow-md flex flex-col items-center justify-center text-center">
//               <DocumentTextIcon className="w-12 h-12 text-orange-500 mb-3" />
//               <h2 className="text-xl font-semibold text-secondary mb-2">Pending Verification</h2>
//               <p className="text-4xl font-bold text-orange-500">{dashboardData.stats.pendingVerifications}</p>
//             </div>

//             {/* Total Applications Card - Now Clickable */}
//             <div
//               className="bg-white p-6 rounded-lg shadow-md cursor-pointer hover:shadow-lg transition-shadow duration-300 flex flex-col items-center justify-center text-center"
//               onClick={handleViewApplications}
//             >
//               <ChartBarIcon className="w-12 h-12 text-purple-600 mb-3" />
//               <h2 className="text-xl font-semibold text-secondary mb-2">Total Applications</h2>
//               {applicationsLoading ? (
//                   <p className="text-lg text-gray-500">Loading...</p>
//               ) : applicationsError ? (
//                   <p className="text-sm text-red-500">{applicationsError}</p>
//               ) : (
//                   <p className="text-4xl font-bold text-purple-600">{totalApplications}</p>
//               )}
//               <p className="text-sm text-gray-500 mt-2">Click to view application report</p>
//             </div>
//           </div>
...
//             <div className="mb-8">
//               <h2 className="text-2xl font-bold text-secondary mb-4 border-b pb-2">Post New Content</h2>
...
//                   { label: "Workshop", type: "workshop", endpoint: "/api/workshops/create" }, // ADDED: Workshop option
//                 ].map((item) => (
//                   <button
//                     key={item.type}
//                     onClick={() => handlePostNew(item.type)}
//                     className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg shadow-md flex items-center justify-center transition-colors duration-200"
//                   >
//                     <PlusIcon className="w-5 h-5 mr-2" /> Post {item.label}
//                   </button>
//                 ))}
//               </div>
//             </div>
...
import { useNavigate } from 'react-router-dom';
import PostContentModal from '../components/PostContentModal';
// Using outline icons for a modern, lighter feel
...
const AdminDashboard = () => {
  ...
  // Define the color scheme - Adjusted for more vibrancy and impact
  const primaryNavyBlue = "text-[#1E3A5F]";
  const secondarySteelBlue = "text-[#4A789C]";
  const accentOrange = "text-[#FF6B35]"; // Used for interactive elements
  ...
  const handleAuthError = (err) => {
    if (err.response && err.response.status === 401) {
      ...
      logoutUser(); // Call your logout service
      setUser(null); // Clear user in context
      navigate('/login'); // Redirect to login
      return true; // Indicate that an auth error was handled
    }
  ...
  return (
    <div className={`min-h-screen p-8 ${lightGrayBackground}`}>
        ...
        {dashboardData ? (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
              {/* Total Users Card - Now Clickable & Energetic */}
              <div
              ...
              {/* Pending Verification Card - Energetic */}
              <div className={`p-8 rounded-xl shadow-lg border border-gray-200 
                            transform transition-all duration-300 hover:scale-103 hover:shadow-xl 
              ...
              {/* Total Applications Card - Clickable & Energetic */}
              <div
                className={`p-8 rounded-xl shadow-lg border border-gray-200 cursor-pointer 
                            transform transition-all duration-300 hover:scale-103 hover:shadow-xl 
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`bg-blue-600 hover:bg-blue-700 text-white font-semibold py-4 px-6 rounded-lg shadow-md 
                                  ...
                                  animate-slide-in-bottom delay-${500 + index * 70}`} // Increased stagger delay
                    >
      ...
      {/* Custom CSS for animations and improved look */}
      <style>{`
        @keyframes fadeIn {
          from { opacity: 0; }
          to { opacity: 1; }
        ...
        @keyframes fadeInUp {
          from { opacity: 0; transform: translateY(30px); } /* Increased translation for more impact */
          to { opacity: 1; transform: translateY(0); }
        }
        @keyframes zoomIn {
          from { transform: scale(0.9); opacity: 0; } /* Zoom in from smaller */
          to { transform: scale(1); opacity: 1; }
        }
        @keyframes slideInBottom {
          from { opacity: 0; transform: translateY(20px); } /* Increased translation */
          to { opacity: 1; transform: translateY(0); }
        }
        @keyframes bounceIcon { /* New animation for icons */
          0%, 100% { transform: translateY(0); }
          50% { transform: translateY(-8px); }
        }

        .animate-fade-in-up { animation: fadeInUp 0.7s ease-out forwards; } /* Slightly longer duration for smoothness */
        ...
        .animate-bounce-icon { animation: bounceIcon 1.5s ease-in-out infinite; } /* Apply to icons */


        /* Delay utilities for staggered animations - adjusted for smoother flow */
        .delay-100 { animation-delay: 0.1s; }
        .delay-200 { animation-delay: 0.2s; }
        .delay-300 { animation-delay: 0.3s; }
        .delay-400 { animation-delay: 0.4s; }
        .delay-500 { animation-delay: 0.5s; }
        .delay-570 { animation-delay: 0.57s; } /* Custom delays for button staggering */
