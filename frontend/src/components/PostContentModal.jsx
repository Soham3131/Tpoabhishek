...
const PostContentModal = ({ contentType, onClose }) => {
    ...
    const getEndpoint = () => {
        switch (contentType) {
            ...
            case 'workshop': return "/api/workshops";
            case 'podcast': return "/api/podcasts";
            case 'corporate-event': return "/api/corporate-events"; // ✅ ADDED THIS LINE
            default: return "";
        }
    };
    ...
    const getFormFields = () => {
        switch (contentType) {
            ...
            case 'podcast': return [
                { name: "title", label: "Podcast Title", type: "text", required: true },
                { name: "description", label: "Description", type: "textarea", required: true },
                { name: "youtubeLink", label: "YouTube Link (Video/Playlist URL)", type: "url", required: true },
                { name: "podcastImage", label: "Podcast Image", type: "file", required: true, accept: "image/*" },
            ];
            case 'corporate-event': return [ // ✅ ADDED THIS NEW CASE
                { name: "title", label: "Event Title", type: "text", required: true },
                { name: "hostCompany", label: "Host Company", type: "text", required: true },
                { name: "date", label: "Event Date", type: "date", required: true },
                { name: "location", label: "Location", type: "text", required: true },
                { name: "description", label: "Description", type: "textarea", required: true },
                { name: "registrationLink", label: "Registration Link", type: "url" },
                { name: "speakers", label: "Speakers (comma-separated)", type: "text" },
            ];
            default: return [];
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        for (const key in formData) {
            ...
            } else if (key === 'speakers' && formData[key]) { // ✅ ADDED THIS NEW HANDLING
                formData[key].split(',').map(i => i.trim()).filter(Boolean).forEach(item => {
                    dataToSend.append(key + '[]', item);
                });
            } else {
    ...
    const modalTitle = contentType.charAt(0).toUpperCase() + contentType.slice(1);
    const displayModalTitle = contentType === 'jobfair' ? 'Job Fair' : (contentType === 'corporate-event' ? 'Corporate Event' : modalTitle);
