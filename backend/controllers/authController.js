...
//         } else {
//             // User exists but not verified. Resend OTP or allow verification.
...
//             await existingUser.save(); // Save new OTP to existing unverified user
...
//   // This XSRF-TOKEN cookie is set by server.js middleware for *other* routes
...
//     console.log("Updated User:", await User.findOne({ email }));
...
//     user.isVerified = true; // NEW: Set user as verified
//     user.otp = null;
//     user.otpExpires = null;
//     await user.save();
//     console.log("Verify OTP: Account verified successfully for email:", email);

//     // Optional: Log the user in immediately after successful verification
//     // setAuthCookies(res, user._id, user.role); // Uncomment if you want to auto-login
...
//     const user = await User.findById(req.user.id).select("-password"); // Ensure `req.user.id` is correct (was `req.user._id` in authroutes earlier)
//     if (!user) return res.status(404).json({ msg: "User not found" });
...
//   console.log("updateUserProfile: Received req.body:", req.body); // Check non-file fields
//   console.log("updateUserProfile: Received req.files:", req.files)
//   try {
//     const userId = req.user.id; // Get ID from authenticated user
//     const { name, email } = req.body; // Data from request body
...
//     // Only allow email change if the new email is not already taken by another verified user
...
//       }
//       user.email = email;
//       // If email changes, you might want to re-verify it.
//       // For simplicity here, we'll assume email changes are handled carefully,
//       // or implement re-verification flow if needed.
//     }
...
//       const cloudinaryResponse = await uploadImageToCloudinary(profilePictureFile, "profile_pictures"); // 'profile_pictures' is your folder in Cloudinary
//       user.profilePicture = cloudinaryResponse.secure_url; // Save the secure URL from Cloudinary
